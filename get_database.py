...
for el in educateurs_data:
    # location = geolocator.geocode(el["address"])
    rounded_google_review = round(el["Note"] * 2) / 2
    id_name = el["Nom éducateur"]. \
        lower(). \
        replace("é", "e"). \
        replace("è", "e"). \
        replace("à", "a"). \
        replace("ï", "i")
    id_name = re.sub(r'[^A-Za-z0-9\s]+', '', id_name). \
        replace("    ", " "). \
        replace("   ", " "). \
        replace("  ", " "). \
        replace(" ", "-")
...
for el in blogs_data:
    blogs_database[el["id"]] = {
        ...
        "meta-description": el["meta-description"],
        "hero-image-url": el["hero-image-url"],
        "article": el["article"],
    }
